...
public class ElasticSearchDruidDataSource extends DruidDataSource {
    ...
    public void registerMbean() {
        if (!mbeanRegistered) {
            AccessController.doPrivileged(new PrivilegedAction<Object>() {
                ...
                @Override
                public Object run() {
                    ObjectName objectName = DruidDataSourceStatManager.addDataSource(com.alibaba.druid.pool.ElasticSearchDruidDataSource.this,
                            com.alibaba.druid.pool.ElasticSearchDruidDataSource.this.name);

                    com.alibaba.druid.pool.ElasticSearchDruidDataSource.this.setObjectName(objectName);
                    com.alibaba.druid.pool.ElasticSearchDruidDataSource.this.mbeanRegistered = true;
    ...
    public void unregisterMbean() {
        if (mbeanRegistered) {
            AccessController.doPrivileged(new PrivilegedAction<Object>() {
                ...
                @Override
                public Object run() {
                    DruidDataSourceStatManager.removeDataSource(com.alibaba.druid.pool.ElasticSearchDruidDataSource.this);
                    com.alibaba.druid.pool.ElasticSearchDruidDataSource.this.mbeanRegistered = false;
                    return null;
                }
            });
        }
    ...
    protected void put(Connection connection) {
        ...
        try {
            holder = new DruidConnectionHolder(com.alibaba.druid.pool.ElasticSearchDruidDataSource.this, connection);
        } catch (SQLException ex) {
            lock.lock();
            try {
                if (createScheduler != null) {
